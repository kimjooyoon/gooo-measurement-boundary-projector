...
func RunV2Conformance(sourcePath, corpusPath, outputDir string) (V2ConformanceSummary, error) {
	...
	for _, entry := range entries {
		...
		if err != nil {
			return V2ConformanceSummary{}, fmt.Errorf("v2 case %s collect: %w", entry.CaseID, err)
		}
		evaluation, err := EvaluateV2(ir, collection, filepath.Join(caseDir, "evaluation.json"))
		if err != nil {
			return V2ConformanceSummary{}, fmt.Errorf("v2 case %s evaluate: %w", entry.CaseID, err)
		}
		...
		if err := WriteText(filepath.Join(caseDir, "human-report.md"), RenderV2HumanReport(evaluation)); err != nil {
			return V2ConformanceSummary{}, err
		}
